...
def filter_reads_on_kraken(reads, outfile, db_location, threads, inverse):
    with open(os.devnull, 'w') as devnull:
        if not inverse:
            subprocess.Popen(["kraken", "--db", db_location, "--threads", threads, "--quick", "--min-hits", "1", "--classified-out", outfile, reads], stdout=devnull, stderr=subprocess.PIPE)
        else:
            subprocess.Popen(["kraken", "--db", db_location, "--threads", threads, "--quick", "--min-hits", "1", "--unclassified-out", outfile, reads], stdout=devnull, stderr=subprocess.PIPE)
