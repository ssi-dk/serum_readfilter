...
def add_kraken_id_to_contigs(fasta_file, db_location, kmer_size=21):
    ...
    subprocess.Popen("kraken-build --add-to-library kraken.fasta --db .".split(' '))
    subprocess.Popen("kraken-build --build --kmer-len {} --minimizer-len {} --db .".format(kmer_size, kmer_size - 1).split(' '))
